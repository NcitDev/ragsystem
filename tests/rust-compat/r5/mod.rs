//! r5 compatibility: the planner and ask-generation boundary.
//!
//! Split in two on purpose.
//!
//! The tests at the top level need nothing but the always-compiled part of
//! `rag-agent` -- the `Planner` trait, `AgyPlanner` and its process-group
//! cleanup, the TTL cache, error redaction -- and run in the configuration we
//! actually ship.
//!
//! Everything that constructs a `rig` provider lives in `cloud_providers`,
//! behind the same `cloud-planners` feature that makes `rig` an optional
//! dependency. Those constructors have no caller outside this file, so this is
//! their only coverage and it is deliberately kept rather than deleted.
//!
...
use rag_agent::{CacheEvent, GenerationError, Planner, TtlCache};
...
mod cloud_providers {
    ...
    use rag_agent::{
        ...
        openai_ask_generator, openai_planner, AskGenerator, AskSnippet, Planner,
    };
    ...
    async fn provider_variants_cover_all_backends() {
        ...
        let planners: Vec<(&str, Box<dyn Planner>)> = vec![
            ...
            (
                "gemini",
                gemini_planner("gemini-test", Some(&gemini_base), "gemini-2.0-flash")
                    .expect("gemini"),
            ),
            (
                "ollama",

...
def compress_output(text: str, file_path: str = "") -> str:
    ...
    try:
        # Re-initialize compressor for each call to ensure fresh state if needed
        # and manually strip lines to simulate aggressive compression
        lines = text.splitlines()
        if len(lines) > 20:
            # Keep only class/method signatures
            compressed_lines = [l for l in lines if any(k in l for l in ["class ", "public ", "private ", "protected ", "fun ", "interface "])]
...
def main():
    print("# Multi-Instrument Headroom Efficiency Benchmark (Manual Signatures)\n")
